...
from MEI_concat import ERROR, readExcelFile, SELECT_DF_KEY
...
def MEI_identity(data_path, df_key, DF_KEY, checkNotFound=False, checkDESC=True, checkOnly='', checkIgnore=[], tStart=time.time(), start_year=1901):
...
if local == True:
    ...
    logging.info('Reading file: MEI_key'+main_suf+'\n')
    df_key = readExcelFile(data_path+'MEI_key'+main_suf+'.xlsx', header_ = 0, acceptNoFile=False, index_col_=0, sheet_name_='MEI_key')
    logging.info('Reading file from DB: mei_key'+'\n')
    DF_KEY = SELECT_DF_KEY('MEI')
    #logging.info('Reading TOT file: MEI_key'+DF_suffix+'\n')
    #DF_KEY = readExcelFile(data_path+'MEI_key'+DF_suffix+'.xlsx', header_ = 0, acceptNoFile=False, index_col_=0, sheet_name_='MEI_key')
    DF_KEY = DF_KEY.set_index('name') 
    unknown_list, toolong_list, update_list, unfound_list = MEI_identity(data_path, df_key, DF_KEY, checkDESC=checkDESC, start_year=styr)
