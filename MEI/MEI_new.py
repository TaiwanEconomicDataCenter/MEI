...
from MEI_concat import ERROR, MERGE, NEW_KEYS, CONCATE, UPDATE, readFile, readExcelFile, PRESENT, SELECT_DF_KEY, SELECT_DATABASES, INSERT_TABLES
        ...
        for frequency in frequency_list:
            ...
            if not not unknown_subjects or not not unknown_measures:
                ...
                if not not unknown_subjects:
                    ...
                    for un in unknown_subjects:
                        un_temp = un.replace("00","")
                        if len(un_temp) <= 6:
                            subject_file.loc[un] = un_temp
                            continue
                        un_temp = re.sub(r'^([A-Z])[A-Z][0-9]([0-9])', r"\1\2", un)
                        if len(un_temp) <= 6:
                            subject_file.loc[un] = un_temp
                            continue
            ...
            for i in range(nG):
                ...
                code_num_dict[frequency], table_num_dict[frequency], DATA_BASE_main[frequency], db_table_t_dict[frequency], DB_name_dict[frequency], snl = \
                  MEI_DATA(i, name, MEI_t, code_num_dict[frequency], table_num_dict[frequency], KEY_DATA, DATA_BASE_main[frequency], db_table_t_dict[frequency], DB_name_dict[frequency], snl, FREQLIST[frequency], frequency)  
...
if excel_suffix == 'mysql':
    ...
    for key in DATA_BASE_dict.keys():
        DB_name.append(key)
        if str(key).find('A_') >= 0:
            annual_num += 1
        elif str(key).find('Q_') >= 0:
            quarterly_num += 1
        elif str(key).find('M_') >= 0:
            monthly_num += 1
    if annual_num > 0:
        database_num += 1
    if quarterly_num > 0:
        database_num += int((quarterly_num-1)/maximumQ)+1
    if monthly_num > 0:
        database_num += int((monthly_num-1)/maximumM)+1
    if quarterly_num > maximumQ or monthly_num > maximumM:
        ...
        for d in range(1, database_num+1):
            with pd.ExcelWriter(out_path+NAME+"database_"+str(d)+excel_suffix+".xlsx") as writer: # pylint: disable=abstract-class-instantiated
                ...
                for db in range(current_db, len(DB_name)):
                    ...
                    if db < len(DB_name)-1:
                        if str(DB_name[db]).find('Q_') >= 0:
                            current_quarterly += 1
                        if str(DB_name[db]).find('M_') >= 0:
                            current_monthly += 1
                        if (str(DB_name[db]).strip()[:4] != str(DB_name[db+1]).strip()[:4]) or (current_quarterly >= maximumQ) or (current_monthly >= maximumM):
                            current_db = db+1
                            break
                writer.save()
                sys.stdout.write("\n")
